...
public class FastBase58 {
    ...
    public static void encode58LexP4(long high, long low, byte[] out, int offset) {
        ...
        int index = offset + 20;
        while (index > offset) {
            int remainder = divmod58p4(out, firstBitTimes2 >>> 6, 30);
            int remainderHigh = fastdiv_u32(remainder, M58p2);
            int remainderLow = fastmod_u32(remainder, M58p2, 58 * 58);
    ...
    private static int divmod58p4(byte[] number, int firstDigit, int baseShift) {
        // this is just long division which accounts for the base of the input digits
        long remainder = 0;
        for (int index = (3 - firstDigit) << 2; index >= 0; index -= 4) {
            int digit = (int) readIntLE.get(number, index);
            long temp = (remainder << baseShift) + digit;
            readIntLE.set(number, index, (int) (temp / (58 * 58 * 58 * 58)));
            remainder = temp % (58 * 58 * 58 * 58);
        }
        return (int) remainder;
    }
