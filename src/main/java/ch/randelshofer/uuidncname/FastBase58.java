...
public class FastBase58 {
    ...
    private static long M58p4 = computeM_u32(58 * 58 * 58 * 58);
    ...
    public static void encode58(long high, long low, byte[] out, int offset) {
        ...
        int leadingZeros = 0;
        while (index > offset) {
            int remainder = divmodExpected(out, firstBitTimes2 >>> 6, 30, 58 * 58 * 58 * 58, M58p4);
            ...
            int d2 = fastmod_u32(remainderHigh, M58, 58);
            int d3 = fastdiv_u32(remainderHigh, M58);
            ...
            if (d3 == 0) leadingZeros++;
            else leadingZeros = 0;
            out[index--] = ALPHABET[d0];
            out[index--] = ALPHABET[d1];
            out[index--] = ALPHABET[d2];
            out[index--] = ALPHABET[d3];
            firstBitTimes2 += 37;
        }
        if (out[0] == 0) leadingZeros++;
        out[index] = ALPHABET[out[0]];
     ...
     * @param offset the offset in the output array
     */
    public static void encode58LexP4(long high, long low, byte[] out, int offset) {
        // Store the number in little-endian order in the first 16 bytes of the output array.
        // We distribute the 120 bits into 4 int values - storing 30 bits in each.
        int mask30 = (1 << 30) - 1;
        readIntLE.set(out, 12, (int) (high >> 30));
        ...
        while (index > offset) {
            ...
            out[index--] = ALPHABET[fastmod_u32(remainderHigh, M58, 58)];
            out[index--] = ALPHABET[fastdiv_u32(remainderHigh, M58)];
            firstBitTimes2 += 37;
        }
        out[index] = ALPHABET[out[0]];
    }
    ...
    public static void encode58Lex(long high, long low, byte[] out, int offset) {
        encode58LexP4(high, low, out, offset);
    }

    public static void encode58LexP3(long high, long low, byte[] out, int offset) {
        // Store the number in little-endian order in the first 16 bytes of the output array.
        // We distribute the 120 bits into 4 int values - storing 30 bits in each.
        int mask30 = (1 << 30) - 1;
        readIntLE.set(out, 12, (int) (high >> 30));
    ...
    private static int divmodExpected(byte[] number, int firstDigit, int baseShift, int divisor, long M) {
        // this is just long division which accounts for the base of the input digits
        long remainder = 0;
        for (int index = (3 - firstDigit) << 2; index >= 0; index -= 4) {
            int digit = (int) readIntLE.get(number, index);
            long temp = (remainder << baseShift) + digit;
            readIntLE.set(number, index, (int) (temp / divisor));
